...
describe('validateRequest', () => {
  ...
  it('accepts valid input with genre', () => {
    const result = validateRequest({ month: 3, day: 13, genre: 'Buried Pirate Loot' });
    expect(result.valid).toBe(true);
    if (result.valid) {
      expect(result.data).toEqual({ month: 3, day: 13, genre: 'Buried Pirate Loot' });
    }
  });
...
describe('buildUserMessage', () => {
  ...
  it('includes storytelling guidance in genre instruction', () => {
    ...
    expect(msg).toContain('tone, pacing, and humor style');
    expect(msg).toContain('absurd details');
